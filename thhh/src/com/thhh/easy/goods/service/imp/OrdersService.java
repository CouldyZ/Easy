...
public class OrdersService implements IOrdersService {
	...
	public void deleteOrders(Integer ordersId) {
		//删除订单详情表中的商品
		String hql = "delete from Orderdetail ord where ord.orders.id=?" ;
		Session session = orderdetailDAO.getHibernateTemplate().getSessionFactory().openSession();
		Query query = session.createQuery(hql); 
		query.setInteger(0, ordersId) ;
		query.executeUpdate() ;
		session.close();
		//删除订单
		ordersDao.delete(findOrders(ordersId)) ;
	}
